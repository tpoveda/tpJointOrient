...
class tpJointOrient(QDialog, object):
    ...
    def orientJoints(self):
        ...
        for jnt in reversed(joints):
            ...
            if aimTarget != '':
                ...
                cmds.delete(cmds.aimConstraint(aimTarget, jnt, aim=aimAxis, upVector=upAxis, worldUpVector=worldUpAxis, worldUpType='vector', weight=1.0))
    ...
    def manualOrientJoints(self, type):
        ...
        tweakRot = [self.manualJointOriXSpin.value() * tweak, self.manualJointOriYSpin.value() * tweak, self.manualJointOriZSpin.value() * tweak]
        joints = cmds.ls(selection=True, type='joint')
    ...
    def setManualOrientJoints(self):
        ...
        tweakRot = [self.manualJointOriXSpin.value(), self.manualJointOriYSpin.value(), self.manualJointOriZSpin.value()]
        joints = cmds.ls(selection=True, type='joint')
